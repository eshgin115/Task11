...
namespace DemoApplication.Areas.Admin.Controllers
{
    ...
    public class BookController : Controller
    {
        ...
        private readonly MultipleFileService _multipleFileServices;

        public BookController(
            DataContext dataContext, 
            ILogger<BookController> logger,
            MultipleFileService multipleFileServices)
        {
            _dataContext = dataContext;
            _logger = logger;
            _multipleFileServices = multipleFileServices;
        }
        ...
        public async Task<IActionResult> Add(AddViewModel model)
        {
            ...
            List<string> imageNamesInSystem = await _multipleFileServices.UploadAsync(model!.Images, UploadDirectory.Book);
        ...
        public async Task<IActionResult> UpdateAsync([FromRoute] int id)
        {
            ...
            var model = new AddViewModel
            {
                ...
                ImageUrls = _multipleFileServices.GetFileUrl(book.Images.Select(b => new ImageData(b.ImageName)).ToList(), Contracts.File.UploadDirectory.Book)
        ...
        public async Task<IActionResult> UpdateAsync(AddViewModel model)
        {
            ...
            await _multipleFileServices.DeleteAsync(book.Images.Select(b => new ImageData(b.ImageName)).ToList(), UploadDirectory.Book);
            var imageFileNamesInSystem = await _multipleFileServices.UploadAsync(model.Images, UploadDirectory.Book);

            await UpdateBookAsync(model.Images, imageFileNamesInSystem);
            ...
            async Task UpdateBookAsync(List<IFormFile> formFiles, List<string> imageNameInFileSystem)
            {
                ...
                for (int i = 0; i < formFiles.Count; i++)
                {
                    var image = new Image
                    {
                        Book = book,
                        ImageName = formFiles[i].Name,
                        ImageNameInFileSystem = imageNameInFileSystem[i]
                    };
                    _dataContext.Images.Add(image);
                };
